...
public interface AddSignService {
    /**
     * 创建加签任务
     * @param parentTaskId 父任务ID（被加签的任务，UUID格式）
     ...
     * @return 加签记录ID
     */
    Long createAddSign(String parentTaskId, String targetUserId, String targetUserName, String targetEmpCode,
                       String addsignUserId, String addsignUserName, String addsignEmpCode, String addsignReason);

    /**
     * 完成加签任务（加签人审批后，回到原任务）
     * @param addsignTaskId 加签任务ID（UUID格式）
     * @param approvalResult 审批结果：APPROVED-同意，REJECTED-拒绝
     * @param approvalOpinion 审批意见
     * @return 是否成功
     */
    boolean completeAddSign(String addsignTaskId, String approvalResult, String approvalOpinion);

    /**
     * 根据父任务ID查询加签记录
     */
    java.util.List<AddSignRecord> getAddSignRecordsByParentTaskId(String parentTaskId);
}
