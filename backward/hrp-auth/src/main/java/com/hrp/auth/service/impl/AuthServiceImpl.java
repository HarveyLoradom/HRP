...
public class AuthServiceImpl implements AuthService {
    ...
    public Map<String, Object> login(String account, String password, String ip, boolean hasCaptcha) {
        ...
        try {
            forceChangePasswordCode = codeMapper.selectByCodeName("强制修改密码");
        } catch (Exception e) {
            // 如果查询失败，忽略此配置，不影响登录
        ...
        try {
            resetPasswordCode = codeMapper.selectByCodeName("原始密码");
        } catch (Exception e) {
            // 如果查询失败，使用默认密码
        ...
        // 查询员工信息，获取empName、photo、empPhone等
        ...
        try {
            ...
            if (employee != null) {
                ...
                photo = employee.getPhoto(); // 获取照片路径
                empPhone = employee.getEmpPhone(); // 获取手机号
            }
        } catch (Exception e) {
        ...
        result.put("empId", employee != null ? employee.getEmpId() : null); // 员工ID
        result.put("empPhone", empPhone != null ? empPhone : user.getPhone()); // 员工手机号，如果没有则使用user表的phone
        result.put("deptId", user.getDeptId()); // 部门ID（从User对象中获取，已通过关联查询获取）
        result.put("deptName", user.getDeptName()); // 部门名称（从User对象中获取，已通过关联查询获取）
        result.put("photo", photo); // 照片路径
