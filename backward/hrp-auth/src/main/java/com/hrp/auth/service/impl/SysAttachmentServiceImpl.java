...
public class SysAttachmentServiceImpl implements SysAttachmentService {
    ...
    @Override
    @Transactional
    public boolean updateBusinessId(Long attachmentId, Long businessId) {
        return sysAttachmentMapper.updateBusinessIdById(attachmentId, businessId) > 0;
    }

    @Override
    @Transactional
    public boolean updateBusinessIdBatch(String businessType, Long businessId, List<Long> attachmentIds) {
        if (attachmentIds == null || attachmentIds.isEmpty()) {
            return true;
        }
        return sysAttachmentMapper.updateBusinessIdByTypeAndNull(businessType, businessId, attachmentIds) > 0;
    }
