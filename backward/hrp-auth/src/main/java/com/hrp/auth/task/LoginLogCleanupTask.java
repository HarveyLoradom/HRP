...
public class LoginLogCleanupTask {
     ...
     * 配置参数名称
     */
    private static final String CONFIG_CODE_NAME = "登录日志保留天数";
    ...
    private int getRetainDays() {
        try {
            Code config = codeMapper.selectByCodeName(CONFIG_CODE_NAME);
            if (config != null && config.getCodeValue() != null && !config.getCodeValue().trim().isEmpty()) {
                ...
                logger.info("未找到日志保留天数配置（{}），使用默认值：{}", CONFIG_CODE_NAME, DEFAULT_RETAIN_DAYS);
            }
        } catch (Exception e) {
