...
public class BudgetExecutionController {
    ...
    @GetMapping("/{id}")
    public Result<BudgetExecution> getById(@PathVariable("id") Long id) {
        BudgetExecution execution = budgetExecutionService.getById(id);
        return Result.success(execution);
    }

    @GetMapping("/no/{no}")
    public Result<BudgetExecution> getByNo(@PathVariable("no") String no) {
    ...
    public Result<List<BudgetExecution>> getByBudgetId(@PathVariable("budgetId") Long budgetId) {
        List<BudgetExecution> list = budgetExecutionService.getByBudgetId(budgetId);
        return Result.success(list);
    }

    @GetMapping("/business")
    public Result<List<BudgetExecution>> getByBusinessId(@RequestParam(value = "businessType") String businessType, @RequestParam(value = "businessId") Long businessId) {
