...
public class BudgetSubjectController {
    ...
    @GetMapping("/{id}")
    public Result<BudgetSubject> getById(@PathVariable("id") Long id) {
        BudgetSubject subject = budgetSubjectService.getById(id);
        return Result.success(subject);
    }

    @GetMapping("/code/{code}")
    public Result<BudgetSubject> getByCode(@PathVariable("code") String code) {
        BudgetSubject subject = budgetSubjectService.getByCode(code);
        return Result.success(subject);
    }

    @GetMapping("/list")
    public Result<List<BudgetSubject>> getAll(@RequestParam(value = "isStop", required = false) Long isStop) {
        List<BudgetSubject> list = budgetSubjectService.getAll(isStop);
    ...
    public Result<List<BudgetSubject>> getByParentId(@PathVariable("parentId") Long parentId) {
        List<BudgetSubject> list = budgetSubjectService.getByParentId(parentId);
        return Result.success(list);
    }

    @GetMapping("/dept/{deptId}")
    public Result<List<BudgetSubject>> getByDeptId(@PathVariable("deptId") Long deptId) {
    ...
    public Result<List<com.hrp.common.entity.Dept>> getRelatedDepts(@PathVariable("subjectId") Long subjectId) {
