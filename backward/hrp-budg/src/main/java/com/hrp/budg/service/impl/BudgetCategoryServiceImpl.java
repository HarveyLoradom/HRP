...
public class BudgetCategoryServiceImpl implements BudgetCategoryService {
    ...
    @Override
    public PageResult<BudgetCategory> getLevel1Page(Long page, Long size, String budgetYear, String categoryType, Long isStop) {
        PageHelper.startPage(page.intValue(), size.intValue());
        List<BudgetCategory> list = budgetCategoryMapper.selectLevel1ByConditions(budgetYear, categoryType, isStop);
        PageInfo<BudgetCategory> pageInfo = new PageInfo<>(list);
        return new PageResult<>(pageInfo.getList(), pageInfo.getTotal(), size, page);
    }

    @Override
    public PageResult<BudgetCategory> getLevel2Page(Long page, Long size, String budgetYear, String categoryType, Long parentCategoryId, Long isStop) {
        PageHelper.startPage(page.intValue(), size.intValue());
        List<BudgetCategory> list = budgetCategoryMapper.selectLevel2ByConditions(budgetYear, categoryType, parentCategoryId, isStop);
        PageInfo<BudgetCategory> pageInfo = new PageInfo<>(list);
        return new PageResult<>(pageInfo.getList(), pageInfo.getTotal(), size, page);
    }

    @Override
    public List<BudgetCategory> getLevel1List(String budgetYear, String categoryType, Long isStop) {
        return budgetCategoryMapper.selectLevel1ByConditions(budgetYear, categoryType, isStop);
    }

    @Override
    public List<BudgetCategory> getLevel2List(String budgetYear, String categoryType, Long parentCategoryId, Long isStop) {
        return budgetCategoryMapper.selectLevel2ByConditions(budgetYear, categoryType, parentCategoryId, isStop);
    }
    ...
    public boolean stop(Long id) {
        ...
        if (category.getCategoryLevel() == 1) {
            // 一级分类：检查是否有启用的二级分类
            int count = budgetCategoryMapper.countActiveLevel2ByParentId(id);
            if (count > 0) {
                throw new BusinessException("该一级分类下存在启用的二级分类，无法停用。请先停用所有二级分类");
            }
        } else if (category.getCategoryLevel() == 2) {
            // 二级分类：检查是否有启用的项目预算
            int count = budgetCategoryMapper.countActiveItemsByCategoryId(id);
            if (count > 0) {
                throw new BusinessException("该二级分类下存在启用的项目预算，无法停用。请先停用所有项目预算");
            }
        }
    ...
    public String generateLevel1Code(String budgetYear, String categoryType) {
        // 规则：年度(4位) + 预算分类类型前缀(2位) + 序号(2位)
        // 例如：2025SR01（2025年收入预算第一个一级分类，SR=收入，ZC=支出）
        String typePrefix = "";
