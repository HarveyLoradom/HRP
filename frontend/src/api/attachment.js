...
// 根据业务查询附件（兼容旧接口，使用businessType和businessId）
...
// 根据businessId查询附件（新接口，只使用businessId）
export function getAttachmentsByBusinessId(businessId) {
  return request({
    url: `/auth/attachment/business-id?businessId=${businessId}`,
    method: 'get'
  })
}
...
// 删除业务相关附件（兼容旧接口，使用businessType和businessId）
...
// 根据businessId删除附件（新接口，只使用businessId）
export function deleteAttachmentsByBusinessId(businessId) {
  return request({
    url: `/auth/attachment/business-id?businessId=${businessId}`,
    method: 'delete'
  })
