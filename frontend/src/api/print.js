...
// 打印模板管理
export const getPrintTemplateList = (isActive) => {
  const params = isActive !== null && isActive !== undefined ? { isActive } : {}
  return request.get('/auth/print-template/list', { params })
}
...
export const getPrintTemplateByType = (type, isActive) => {
  const params = isActive !== null && isActive !== undefined ? { isActive } : {}
  return request.get(`/auth/print-template/type/${type}`, { params })
}
