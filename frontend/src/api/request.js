...
service.interceptors.request.use(
  config => {
    ...
    // 对于 PUT 和 POST 请求，如果 data 是对象，确保 Content-Type 是 application/json
    if ((config.method === 'put' || config.method === 'post') && config.data && typeof config.data === 'object' && !(config.data instanceof FormData) && !(config.data instanceof URLSearchParams)) {
...
service.interceptors.response.use(
  response => {
    ...
    if (res.code !== 200) {
      ...
      // 使用普通对象而不是Error对象，避免触发Vue的错误覆盖层
      const error = {
        message: res.message || '请求失败',
        response: response,
        code: res.code,
        data: res.data,
        // 添加toString方法，避免显示[object Object]
        toString: function() {
