<template>
  <div class="bpmn-designer-container">
    ...
    <el-dialog
      ...
      @close="handlePropertiesDialogClose"
    >
      <element-properties
...
export default {
  ...
  data() {
    return {
      ...
      originalElementProperties: null, // 保存原始属性，用于取消时恢复
      canUndo: false,
      canRedo: false,
      bpmnXml: '',
  ...
  watch: {
    propertiesDialogVisible(newVal) {
      // 当弹窗打开时，强制重新加载属性
      if (newVal && this.selectedElement && this.selectedElement.id && this.bpmnModeler) {
        this.$nextTick(() => {
          // 从 elementRegistry 重新获取元素，确保获取最新的 businessObject
          const elementRegistry = this.bpmnModeler.get('elementRegistry')
          if (elementRegistry && this.selectedElement && this.selectedElement.id) {
            const latestElement = elementRegistry.get(this.selectedElement.id)
            if (latestElement) {
              // 使用 Object.assign 确保 businessObject 引用被正确保留
              this.selectedElement = Object.assign({}, latestElement, {
    ...
    value: {
      immediate: false,
      handler(newVal) {
        if (this.bpmnModeler && newVal) {
          this.loadBpmnXml(newVal, this.customProperties)
        }
      }
    },
    customProperties: {
      ...
      deep: true,
      handler(newVal) {
        if (this.bpmnModeler && this.value && newVal) {
          this.loadBpmnXml(this.value, newVal)
        }
      }
    }
  },
  mounted() {
    ...
    this.$nextTick(() => {
      setTimeout(() => {
        ...
        if (this.value) {
          this.loadBpmnXml(this.value, this.customProperties)
        } else {
    ...
    initBpmnModeler() {
      ...
      this.bpmnModeler.on('selection.changed', (e) => {
        ...
        if (newElement) {
          // 从 elementRegistry 重新获取元素，确保获取最新的 businessObject
          const elementRegistry = this.bpmnModeler.get('elementRegistry')
          const latestElement = elementRegistry.get(newElement.id)
          if (latestElement) {
            // 使用 Object.assign 确保 businessObject 引用被正确保留
            this.selectedElement = Object.assign({}, latestElement, {
              businessObject: latestElement.businessObject
            ...
            this.selectedElement = Object.assign({}, newElement, {
              businessObject: newElement.businessObject
            })
          }
        } else {
      ...
      this.bpmnModeler.on('element.dblclick', (e) => {
        ...
        if (latestElement) {
          ...
          this.selectedElement = Object.assign({}, element, {
            businessObject: element.businessObject
          })
        }
        
        // 保存原始属性
        this.saveOriginalProperties(this.selectedElement)
        // 使用$nextTick确保属性面板组件已更新
        this.$nextTick(() => {
          // 打开属性配置弹窗
...
<bpmn2:definitions xmlns:bpmn2="http://www.omg.org/spec/BPMN/20100524/MODEL" 
  ...
  <bpmndi:BPMNDiagram id="BPMNDiagram_1">
    ...
    async loadBpmnXml(xml, customProperties = null) {
      // 如果XML为空或无效，创建新图表
      if (!xml || !xml.trim() || xml.trim().length === 0) {
        console.warn('XML为空，创建新图表')
        this.createNewDiagram()
        return
      }
      ...
      try {
        ...
        if (customProperties && typeof customProperties === 'object') {
          this.$nextTick(() => {
            this.restoreCustomProperties(customProperties)
          })
        }
    ...
    // 恢复自定义属性
    restoreCustomProperties(customProperties) {
      try {
        const elementRegistry = this.bpmnModeler.get('elementRegistry')
        const moddle = this.bpmnModeler.get('moddle')
        
        Object.keys(customProperties).forEach(elementId => {
          const element = elementRegistry.get(elementId)
          if (!element || !element.businessObject) {
            console.warn('无法找到元素:', elementId)
            return
          }
          
          const props = customProperties[elementId]
          const businessObject = element.businessObject
          
          // 直接设置所有属性到 businessObject
          Object.keys(props).forEach(key => {
            // 特殊处理 conditionExpression
            if (key === 'conditionExpression' && element.type && element.type.includes('SequenceFlow')) {
              if (props.conditionExpression && props.conditionType !== 'none') {
                const conditionExpression = moddle.create('bpmn:FormalExpression', {
                  body: props.conditionExpression
                })
              ...
              // 其他属性直接设置
              businessObject[key] = props[key]
            }
          })
          
        })
      } catch (error) {
        console.error('恢复自定义属性失败:', error)
      }
    },
      ...
      try {
        ...
        elements.forEach(element => {
          ...
          if (element.type && element.type.includes('UserTask')) {
            if (businessObject.approvalType) customProps.approvalType = businessObject.approvalType
            if (businessObject.assigneeType) customProps.assigneeType = businessObject.assigneeType
            if (businessObject.assigneeId) customProps.assigneeId = businessObject.assigneeId
            if (businessObject.assigneeName) customProps.assigneeName = businessObject.assigneeName
            if (businessObject.assigneeCode) customProps.assigneeCode = businessObject.assigneeCode
            if (businessObject.positionCode) customProps.positionCode = businessObject.positionCode
            if (businessObject.deptCode) customProps.deptCode = businessObject.deptCode
            if (businessObject.responsibleType) customProps.responsibleType = businessObject.responsibleType
            if (businessObject.multiInstanceType) customProps.multiInstanceType = businessObject.multiInstanceType
            if (businessObject.multiInstanceCount !== undefined) customProps.multiInstanceCount = businessObject.multiInstanceCount
            if (businessObject.completionCondition !== undefined) customProps.completionCondition = businessObject.completionCondition
            if (businessObject.allowAddsign !== undefined) customProps.allowAddsign = businessObject.allowAddsign
            if (businessObject.allowTransfer !== undefined) customProps.allowTransfer = businessObject.allowTransfer
            if (businessObject.allowReject !== undefined) customProps.allowReject = businessObject.allowReject
            if (businessObject.rejectStrategy) customProps.rejectStrategy = businessObject.rejectStrategy
            if (businessObject.needPrint !== undefined) customProps.needPrint = businessObject.needPrint
            if (businessObject.printOrder !== undefined) customProps.printOrder = businessObject.printOrder
            if (businessObject.enableTimeout !== undefined) customProps.enableTimeout = businessObject.enableTimeout
            if (businessObject.timeoutHours !== undefined) customProps.timeoutHours = businessObject.timeoutHours
            if (businessObject.timeoutAction) customProps.timeoutAction = businessObject.timeoutAction
            if (businessObject.description) customProps.description = businessObject.description
          }
          ...
          if (element.type && (element.type.includes('Gateway') || element.type.includes('ExclusiveGateway') || element.type.includes('ParallelGateway'))) {
            if (businessObject.gatewayType) customProps.gatewayType = businessObject.gatewayType
            if (businessObject.description) customProps.description = businessObject.description
          }
          
          // 连线的自定义属性
          if (element.type && element.type.includes('SequenceFlow')) {
            if (businessObject.conditionType) customProps.conditionType = businessObject.conditionType
            if (businessObject.conditionExpression) {
              if (typeof businessObject.conditionExpression === 'string') {
                customProps.conditionExpression = businessObject.conditionExpression
              } else if (businessObject.conditionExpression.body) {
                customProps.conditionExpression = businessObject.conditionExpression.body
              }
            }
          }
          ...
          if (Object.keys(customProps).length > 0) {
            customProperties[element.id] = customProps
          }
        })
        
        return { xml, customProperties }
      } catch (err) {
    ...
    async handlePreview() {
      try {
        ...
        const { xml } = await this.bpmnModeler.saveXML({ format: true })
        if (!xml) {
          this.$message.error('获取XML失败')
          return
        }
        
        // 获取自定义属性
        const jsonData = await this.getBpmnJson()
        const customProperties = jsonData ? jsonData.customProperties : {}
        
        // 构建完整的预览内容：XML + 自定义属性
        let previewContent = xml
        
        // 如果有自定义属性，在 XML 后面追加自定义属性信息
        if (customProperties && Object.keys(customProperties).length > 0) {
          previewContent += '\n\n<!-- ============================================ -->\n'
          previewContent += '<!-- 自定义属性 (Custom Properties) -->\n'
          previewContent += '<!-- ============================================ -->\n'
          previewContent += JSON.stringify(customProperties, null, 2)
        }
        
        this.bpmnXml = previewContent
    ...
    async handleSave() {
      try {
        // 获取最新的 XML（包含所有当前的自定义属性）
        const { xml } = await this.bpmnModeler.saveXML({ format: true })
        if (!xml) {
          this.$message.error('获取流程XML失败')
          return
        }
        
        // 获取自定义属性
        const jsonData = await this.getBpmnJson()
        const json = jsonData ? { customProperties: jsonData.customProperties || {} } : { customProperties: {} }
    ...
    saveOriginalProperties(element) {
      if (!element) {
        this.originalElementProperties = null
        return
      }
      ...
      try {
        this.originalElementProperties = {
          ...
          conditionExpression: businessObject.conditionExpression ? 
            (typeof businessObject.conditionExpression === 'string' ? businessObject.conditionExpression : businessObject.conditionExpression.body || '') : ''
        }
      } catch (error) {
        console.error('保存原始属性失败:', error)
        this.originalElementProperties = null
      }
    },

    // 属性更新（确定时调用）
    handlePropertyConfirm(updates) {
      ...
      try {
        ...
        // 检查 businessObject
        const businessObject = element ? element.businessObject : null
        if (!businessObject) {
          this.$message.error('保存失败：无法找到元素对象')
          return
        }
      ...
      if (element.type && element.type.includes('UserTask')) {
        ...
        if (updates.assigneeCode !== undefined) {
          propertiesToUpdate.assigneeCode = updates.assigneeCode
        }
        ...
        if (updates.responsibleType !== undefined) {
          propertiesToUpdate.responsibleType = updates.responsibleType
        }
        // 会签配置
      ...
      if (updates.description !== undefined && !propertiesToUpdate.description) {
        ...
        // 直接更新 businessObject 的属性，确保所有自定义属性都能保存
        // businessObject 已经在上面声明过了，这里直接使用
        
        // 先使用 updateLabel 更新名称（标签）
        if (updates.name !== undefined && updates.name !== null) {
          modeling.updateLabel(element, updates.name)
        }
        
        // 直接设置所有自定义属性到 businessObject
        Object.keys(propertiesToUpdate).forEach(key => {
          businessObject[key] = propertiesToUpdate[key]
        })
        
        // 确保名称也被保存到 businessObject（如果还没有通过 updateLabel 设置）
        if (updates.name !== undefined && updates.name !== null) {
          businessObject.name = updates.name
        }
        
        this.$message.success('属性保存成功')
        
        // 立即更新 selectedElement，确保下次打开属性面板时能读取到最新值
        // 从 elementRegistry 重新获取元素，确保获取最新的 businessObject
        if (this.selectedElement && this.selectedElement.id) {
          const updatedElement = elementRegistry.get(this.selectedElement.id)
          if (updatedElement && updatedElement.businessObject) {
            // 更新整个元素的引用，触发 watch
            // 使用 Object.assign 确保 businessObject 引用被正确保留
            this.selectedElement = Object.assign({}, updatedElement, {
              businessObject: updatedElement.businessObject
            })
          }
        }
    ...
    handlePropertyCancel() {
      // 取消时不做任何操作，直接关闭弹窗
      // 因为我们在 localElement 中保持了原始值，取消时不需要恢复
      this.propertiesDialogVisible = false
    },

    // 属性弹窗关闭
    handlePropertiesDialogClose() {
      this.selectedElement = null
      this.originalElementProperties = null
    },
