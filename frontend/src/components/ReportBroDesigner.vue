    ...
    loadTemplate() {
      if (this.templateJson) {
        try {
          ...
          // 如果templateJson中包含fieldGroups，优先使用
          if (data.fieldGroups) {
      ...
      // 从templateFields加载fieldGroups（如果templateJson中没有）
      if (this.templateFields && this.fieldGroups.length === 0) {
        try {
          const fieldGroupsData = typeof this.templateFields === 'string' ? JSON.parse(this.templateFields) : this.templateFields
          if (Array.isArray(fieldGroupsData)) {
            this.fieldGroups = fieldGroupsData
          }
        } catch (e) {
          console.error('加载字段组失败:', e)
        }
      }
    },
