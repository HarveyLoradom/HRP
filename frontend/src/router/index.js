...
const routes = [
  ...
  },
  {
    ...
    children: [
      ...
      {
        path: '/hrp/business/password-change-log',
        name: 'PasswordChangeLog',
        component: () => import('../views/system/PasswordChangeLog.vue')
      },
      // 智能报账 - 业务办理
      {
      ...
      {
        path: '/hrp/budg/adjustment-query',
        name: 'BudgetAdjustmentQuery',
        component: () => import('../views/budg/BudgetAdjustmentQuery.vue')
      },
      // 全景人力 - 个人业务
      {
      ...
      {
        path: '/hrp/hr/attendance/query',
        name: 'AttendanceQuery',
        component: () => import('../views/hr/AttendanceQuery.vue')
      },
      // 全景人力 - 考勤管理
      {
        path: '/hrp/hr/business-approval',
        name: 'BusinessApproval',
        component: () => import('../views/hr/BusinessApproval.vue')
      },
      {
        path: '/hrp/hr/attendance/statistics',
        name: 'AttendanceStatistics',
        component: () => import('../views/hr/AttendanceStatistics.vue')
      },
      // 全景人力 - 薪酬管理
      {
        path: '/hrp/hr/salary/config',
        name: 'SalaryConfig',
        component: () => import('../views/hr/SalaryConfig.vue')
      },
      {
        path: '/hrp/hr/salary/calculate',
        name: 'SalaryCalculate',
        component: () => import('../views/hr/SalaryCalculate.vue')
      },
      // 成本管理 - 成本归集与台账
      {
        path: '/hrp/cost/collection',
        name: 'CostCollection',
        component: () => import('../views/cost/CostCollection.vue')
      },
      // 成本管理 - 科室成本
      {
        path: '/hrp/cost/dept',
        name: 'CostDept',
        component: () => import('../views/cost/CostDept.vue')
      },
      // 固定资产 - 资产信息管理
      {
        path: '/hrp/asset/category',
        name: 'AssetCategory',
      ...
      },
      {
        path: '/hrp/asset/item',
        name: 'AssetItem',
        component: () => import('../views/asset/AssetItem.vue')
      },
      // 固定资产 - 采购管理
      {
        path: '/hrp/asset/procurement/apply',
        name: 'AssetProcurementApply',
        component: () => import('../views/asset/ProcurementApply.vue')
      },
      {
        path: '/hrp/asset/procurement/approval',
        name: 'AssetProcurementApproval',
        component: () => import('../views/asset/ProcurementApproval.vue')
      },
      {
        path: '/hrp/asset/procurement/query',
        name: 'AssetProcurementQuery',
        component: () => import('../views/asset/ProcurementQuery.vue')
      },
      {
        path: '/hrp/asset/procurement/storage',
        name: 'AssetProcurementStorage',
        component: () => import('../views/asset/ProcurementStorage.vue')
      },
      // 固定资产 - 资产台账管理
      {
        path: '/hrp/asset/account/query',
        name: 'AssetAccountQuery',
        component: () => import('../views/asset/AssetAccountQuery.vue')
      },
      {
        path: '/hrp/asset/receive/query',
        name: 'AssetReceiveQuery',
        component: () => import('../views/asset/AssetReceiveQuery.vue')
      }
    ]
  }
]
...
router.beforeEach((to, from, next) => {
  ...
  if (requiresAuth) {
    ...
    // 检查路径是否在菜单权限中
    let isInMenus = isPathInMenus(to.path, menus)
    
    // 如果路径不在菜单权限中，检查是否是功能路由（如详情页、编辑页等）
    if (!isInMenus) {
      // 允许访问的功能路由模式（这些路由通常是从父页面跳转过来的）
      const functionalRoutePatterns = [
        /\/purchase-detail$/,
        /\/detail$/,
        /-detail$/,
        /\/edit$/,
      ...
      const isFunctionalRoute = functionalRoutePatterns.some(pattern => pattern.test(to.path))
      
      // 如果是功能路由，检查来源路径是否在菜单权限中
      // 如果来源路径在菜单中，说明是从菜单页面跳转过来的，允许访问功能路由
      if (isFunctionalRoute && from.path && from.path.startsWith('/hrp')) {
        const isFromInMenus = isPathInMenus(from.path, menus)
        if (isFromInMenus) {
          // 来源路径在菜单中，允许访问功能路由
          isInMenus = true
        }
