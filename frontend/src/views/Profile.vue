<template>
  <div v-loading="loading" class="profile-dialog-content">
            ...
            <el-row :gutter="20">
              <el-col :span="24">
                <el-form-item label="手写签名">
                  <div style="display: flex; align-items: flex-start;">
                    <div v-if="signaturePreviewUrl" style="border: 1px solid #dcdfe6; border-radius: 4px; padding: 10px; background: #fff; min-width: 200px; min-height: 100px; display: flex; align-items: center; justify-content: center;">
                      <img :src="signaturePreviewUrl" alt="签名预览" style="max-width: 100%; max-height: 150px; object-fit: contain;" />
                    </div>
                    <div v-else style="border: 1px dashed #dcdfe6; border-radius: 4px; padding: 20px; text-align: center; color: #909399; min-width: 200px; min-height: 100px; display: flex; align-items: center; justify-content: center;">
                      <div>
                        <i class="el-icon-picture" style="font-size: 32px; display: block; margin-bottom: 10px;"></i>
                        <span>暂无签名</span>
                      </div>
                    </div>
                    <div style="margin-left: 20px; flex: 1;">
                      <el-button type="info" icon="el-icon-edit" @click="handleSignatureClick">上传手写签名</el-button>
                      <el-button v-if="signaturePreviewUrl" type="danger" icon="el-icon-delete" @click="handleDeleteSignature" style="margin-left: 10px;">删除签名</el-button>
                      <p style="margin: 10px 0 0 0; color: #909399; font-size: 12px;">签名将用于审批流程中的电子签名</p>
                    </div>
                  </div>
                </el-form-item>
              </el-col>
    ...
    <!-- 手写签名组件 -->
    <SignaturePad v-model="showSignaturePad" @confirm="handleSignatureConfirm" />
  </div>
</template>

<script>
import { getEmployeeByCode, updateEmployee, uploadEmployeePhoto, deleteEmployeePhoto, saveSignature, getCurrentUserSignatureImage, deleteSignature } from '@/api/user'
...
export default {
  ...
  data() {
    return {
      ...
      uploadHeaders: {},
      showSignaturePad: false, // 控制手写签名对话框显示
      signaturePreviewUrl: '', // 签名预览URL
      editForm: {
        empId: null,
        empCode: '',
        empName: '',
  ...
  methods: {
    ...
    async handleSignatureConfirm(base64Image) {
      try {
        const response = await saveSignature(base64Image)
        if (response.code === 200) {
          ...
          this.$message.error(response.message || '签名保存失败')
        }
      } catch (error) {
        console.error('保存签名失败', error)
        this.$message.error('签名保存失败：' + (error.message || '未知错误'))
      }
    },
    async loadSignature() {
      try {
        ...
        if (response.code === 200 && response.data && response.data.imageUrl) {
          // response.data 是一个 Map，包含 imageUrl 字段，格式如：data:image/png;base64,...
          this.signaturePreviewUrl = response.data.imageUrl
        } else {
    ...
    async handleDeleteSignature() {
      try {
        await this.$confirm('确定要删除签名吗？', '提示', {
        ...
        if (response.code === 200) {
          this.$message.success('签名删除成功')
          this.signaturePreviewUrl = ''
        } else {
          this.$message.error(response.message || '签名删除失败')
        }
      } catch (error) {
        if (error !== 'cancel') {
          console.error('删除签名失败', error)
          this.$message.error('签名删除失败：' + (error.message || '未知错误'))
        }
      }
    }
  },
