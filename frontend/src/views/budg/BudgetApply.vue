...
<script>
import { getBudgetApplies, saveBudgetApply, updateBudgetApply, submitBudgetApply } from '@/api/budg'
...
export default {
  ...
  methods: {
    ...
    handleSubmit(row) {
      this.$confirm('确认提交该预算申请吗？', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        submitBudgetApply(row.applyId).then(response => {
          if (response.code === 200) {
    ...
    handleSaveDraft() {
      this.$refs.form.validate(valid => {
        if (valid) {
          ...
          api({ ...this.form, status: 'DRAFT' }).then(response => {
            if (response.code === 200) {
    ...
    handleSaveAndSubmit() {
      this.$refs.form.validate(valid => {
        if (valid) {
          const api = this.isEdit ? updateBudgetApply : saveBudgetApply
          api({ ...this.form, status: 'PENDING' }).then(response => {
            if (response.code === 200) {
              this.$message.success('提交成功')
              this.dialogVisible = false
              this.loadData()
