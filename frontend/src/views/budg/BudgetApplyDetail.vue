          ...
          <el-row :gutter="20">
            ...
            <el-col :span="8">
              <el-form-item label="申请时间:">
                <span>{{ formatDate(detailData.applyDate || detailData.createTime) }}</span>
              </el-form-item>
            </el-col>
            <el-col :span="8">
              <el-form-item label="预算年度:">
                <span>{{ detailData.budgetYear || '-' }}</span>
              </el-form-item>
            </el-col>
          </el-row>
          <el-row :gutter="20">
          ...
          <el-row :gutter="20">
            ...
            <el-col :span="8">
              <el-form-item label="申请人手机:">
                <span>{{ detailData.applicantPhone || '-' }}</span>
              </el-form-item>
            </el-col>
          </el-row>
          <el-row :gutter="20">
...
<script>
import { getBudgetApplyById, withdrawBudgetApply, deleteBudgetApply, submitBudgetApply, approveBudgetApply, rejectBudgetApply, returnBudgetApply } from '@/api/budg'
...
export default {
  ...
  props: {
    ...
    sourceType: {
      type: String,
      required: true,
      validator: value => ['apply', 'approval'].includes(value)
    },
    applyId: {
      type: [Number, String],
      required: false,
  ...
  data() {
    return {
      ...
      previewImageUrl: '',
      // 文件预览改为新窗口打开（图2效果），不再维护 filePreviewVisible / previewFileUrl 等状态
    }
  },
  computed: {
    visible: {
  ...
  methods: {
    ...
    async loadDetail() {
      try {
        const response = await getBudgetApplyById(this.applyId)
        if (response.code === 200 && response.data) {
          this.detailData = response.data
          if (this.detailData.applyNo) {
            await this.loadCompletedNodes(this.detailData.applyNo)
    ...
    async handleSubmit() {
      if (!this.detailData || !this.detailData.applyId) {
      ...
      try {
        const response = await submitBudgetApply(this.detailData.applyId)
    ...
    async handleWithdraw() {
      this.$confirm('确认撤回该预算申请吗？', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(async () => {
        try {
          const response = await withdrawBudgetApply(this.detailData.applyId)
          if (response.code === 200) {
    ...
    async handleDelete() {
      ...
      this.$confirm('确认删除该预算申请吗？删除后无法恢复！', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(async () => {
        try {
          const response = await deleteBudgetApply(this.detailData.applyId)
          if (response.code === 200) {
    ...
    async handleConfirmApprove(data) {
      try {
        ...
        const response = await approveBudgetApply(this.detailData.applyId, opinion, signature)
        if (response.code === 200) {
    ...
    async handleConfirmReturn(data) {
      try {
        ...
        const response = await returnBudgetApply(this.detailData.applyId, returnType, opinion)
        if (response.code === 200) {
    ...
    async handlePrint() {
      ...
      if (this.detailData.printTemplateId) {
        ...
        try {
          ...
          if (!templateId) {
            const defaultTemplateResponse = await getDefaultPrintTemplate('BUDGET_TYPE')
