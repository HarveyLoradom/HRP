          ...
          <el-row :gutter="20">
            ...
            <el-col :span="8">
              <el-form-item label="申请金额:">
                <span style="font-weight: bold; color: #409EFF;">¥{{ detailData.applyAmount }}</span>
              </el-form-item>
            </el-col>
          </el-row>
          <el-row :gutter="20">
...
export default {
  ...
  methods: {
          ...
          for (const item of details) {
            ...
            if (remainingAmount === null && item.subjectId && item.itemId) {
              try {
                ...
                console.error('获取剩余可执行金额失败:', error)
              }
            }
    ...
    async handleDelete() {
      ...
      this.$confirm('确认删除该申请吗？删除后无法恢复！', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(async () => {
    ...
    async handlePrint() {
      ...
      if (this.detailData.printTemplateId) {
        ...
        try {
          ...
          if (!templateId) {
            const defaultTemplateResponse = await getDefaultPrintTemplate('APPLY_TYPE')
