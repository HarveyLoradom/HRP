...
<script>
import { getPayoutList, approvePayout, rejectPayout, returnPayout } from '@/api/reimb'
  ...
  methods: {
    ...
    loadData() {
      ...
      const params = {
        ...
        billTypePrefix: 'BZD', // 只显示报账单（BZD前缀）
    ...
    async handleBatchApprove() {
      if (this.selectedRows.length === 0) {
        this.$message.warning('请选择要审批的报账')
        return
      }
      
      const firstRow = this.selectedRows[0]
      if (!firstRow || !firstRow.payoutBillcode) {
      ...
      this.nextNodeInfo = null
      try {
        const response = await getNextNodeInfoByBusinessKey(firstRow.payoutBillcode)
        if (response.code === 200 && response.data) {
    ...
    async handleBatchReject() {
      if (this.selectedRows.length === 0) {
        this.$message.warning('请选择要拒绝的报账')
        return
      }
    ...
    async doApprove(row, opinion, signature) {
      try {
        ...
        const response = await approvePayout(row.payoutId, userId, opinion, signature)
    ...
    async doReject(row, opinion) {
      try {
        const userId = this.$store.state.user.userInfo.userId || this.$store.state.user.userInfo.id
        const response = await rejectPayout(row.payoutId, userId, opinion)
        if (response.code === 200) {
    ...
    async handleConfirmReturn(data) {
      ...
      if (this.batchRejectRows && this.batchRejectRows.length > 0) {
        ...
        for (const row of this.batchRejectRows) {
          try {
            const response = await returnPayout(row.payoutId, returnType, opinion)
            if (response.code === 200) {
              successCount++
            } else {
              failCount++
