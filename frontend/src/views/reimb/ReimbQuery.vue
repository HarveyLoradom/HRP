          ...
          <el-col :span="6">
            <el-form-item label="状态:" label-width="100px">
              <el-select v-model="searchForm.status" placeholder="请选择状态" clearable style="width: 100%;">
                <el-option
                  v-for="option in payoutStatusOptions"
                  :key="option.value"
                  :label="option.label"
                  :value="option.value"
                ></el-option>
...
export default {
  ...
  methods: {
    async loadCodeTypeOptions() {
      ...
      this.payoutStatusOptions = await getCodeTypeOptions('APPLY_STATUS')
    },
    loadData() {
      this.loading = true
      const params = {
    ...
    getStatusText(status) {
      const option = this.payoutStatusOptions.find(item => item.value === status)
    ...
    handleExportCommand(command) {
      if (command === 'current') {
        this.handleExportReimb(false)
      } else if (command === 'all') {
        this.handleExportReimb(true)
      }
    },
    // 导出报账数据
    async handleExportReimb(exportAll) {
      ...
      try {
        ...
        // 构建表头
        const headers = ['报账单号', '申请人', '部门', '报账类型', '报账金额', '状态', '报账时间']
        ...
        const response = await exportExcel({
          fileName: '报账查询' + (exportAll ? '_全部' : '_第' + this.pagination.page + '页'),
          headers: headers,
          dataList: dataList
        })
        ...
        link.href = url
        link.download = '报账查询' + (exportAll ? '_全部' : '_第' + this.pagination.page + '页') + '.xlsx'
