<template>
  <div class="reimb-query">
    <el-card>
      ...
      <el-form :inline="true" :model="searchForm" class="search-form">
        <el-form-item label="报账单号">
          <el-input v-model="searchForm.billcode" placeholder="请输入报账单号" clearable @keyup.enter.native="handleSearch"></el-input>
        </el-form-item>
    ...
    <!-- 查看详情对话框 -->
    <el-dialog title="报账单详情" :visible.sync="detailVisible" width="600px">
  ...
  methods: {
    ...
    loadData() {
      ...
      getAllPayoutsPage(this.pagination.page, this.pagination.size).then(response => {
        if (response.code === 200 && response.data) {
          ...
          // 只显示报账单
          records = records.filter(item => item.billType === 'PAYOUT')
          // 前端过滤（如果数据量大可以移到后端）
    ...
    async handleViewProcess(row) {
      if (!row.processInstanceId) {
        this.$message.warning('该报账单尚未启动流程')
        return
      }
