...
<el-input v-model="searchForm.deptName" placeholder="请输入部门名称" clearable @keyup.enter.native="handleSearch"></el-input>
      ...
      >
        ...
        <el-table-column label="护士长" width="150">
          <template slot-scope="scope">
            <span v-if="scope.row.nurseManagerName">{{ scope.row.nurseManagerName }}({{ scope.row.nurseManagerCode }})</span>
            <span v-else style="color: #999;">未设置</span>
          </template>
        </el-table-column>
        <el-table-column label="分管院长" width="150">
          <template slot-scope="scope">
            <span v-if="scope.row.vicePresidentName">{{ scope.row.vicePresidentName }}({{ scope.row.vicePresidentCode }})</span>
            <span v-else style="color: #999;">未设置</span>
          </template>
        </el-table-column>
    ...
    <el-dialog :title="dialogTitle" :visible.sync="dialogVisible" width="500px">
      <el-form :model="form" :rules="rules" ref="form" label-width="100px">
        ...
        <el-form-item label="护士长" prop="nurseManagerId">
          <el-autocomplete
            v-model="form.nurseManagerCode"
            :fetch-suggestions="searchEmployee"
            placeholder="请输入工号或姓名搜索"
            value-key="empCode"
            clearable
            @select="handleNurseManagerSelect"
            @clear="handleNurseManagerClear"
            style="width: 100%"
          >
            <template slot-scope="{ item }">
              <div>{{ item.empCode }} - {{ item.empName }}</div>
        ...
        <el-form-item label="分管院长" prop="vicePresidentId">
          <el-autocomplete
            v-model="form.vicePresidentCode"
            :fetch-suggestions="searchEmployee"
            placeholder="请输入工号或姓名搜索"
            value-key="empCode"
            clearable
            @select="handleVicePresidentSelect"
            @clear="handleVicePresidentClear"
            style="width: 100%"
          >
            <template slot-scope="{ item }">
              <div>{{ item.empCode }} - {{ item.empName }}</div>
...
export default {
  ...
  methods: {
    ...
    handleEmployeeClear() {
      this.form.deptManagerId = null
      this.form.deptManagerCode = ''
    },
    handleNurseManagerSelect(item) {
      this.form.nurseManagerId = item.empId
      this.form.nurseManagerCode = item.empCode
    },
    handleNurseManagerClear() {
      this.form.nurseManagerId = null
      this.form.nurseManagerCode = ''
    },
    handleVicePresidentSelect(item) {
      this.form.vicePresidentId = item.empId
      this.form.vicePresidentCode = item.empCode
    },
    handleVicePresidentClear() {
      this.form.vicePresidentId = null
      this.form.vicePresidentCode = ''
    },
    loadData() {
      getDeptList(this.searchForm.isStop !== null && this.searchForm.isStop !== '' ? this.searchForm.isStop : null).then(response => {
        if (response.code === 200) {
    ...
    handleEdit(row) {
      ...
      this.form = {
        ...
        nurseManagerId: row.nurseManagerId,
        nurseManagerCode: row.nurseManagerCode || '',
        vicePresidentId: row.vicePresidentId,
        vicePresidentCode: row.vicePresidentCode || ''
      }
      this.dialogVisible = true
    },
    handleDelete(row) {
