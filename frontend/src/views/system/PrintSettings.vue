  ...
  <div class="print-settings">
    ...
    >
      <el-form :model="form" :rules="rules" ref="form" label-width="120px" style="margin-top: 20px;">
        <el-form-item label="模板编码" prop="templateCode">
          <el-input v-model="form.templateCode" placeholder="请输入模板编码（唯一）"></el-input>
        </el-form-item>
    ...
    >
      <ReportBroDesigner
        ...
        :template-fields="currentDesignTemplate.templateFields"
...
export default {
  ...
  methods: {
    ...
    loadData() {
      ...
      const api = this.searchForm.templateType ? getPrintTemplateByType(this.searchForm.templateType, isActive) : getPrintTemplateList(isActive)
      api.then(response => {
        if (response.code === 200) {
          // 后端已经根据状态和类型进行了筛选，直接使用返回的数据
    ...
    handleExport(row) {
      ...
      exportPrintTemplate(row.templateId).then(response => {
        ...
        link.href = url
        link.download = `${row.templateCode || 'template'}.json`
