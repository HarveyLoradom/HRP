    ...
    <el-card>
      ...
      <el-form :inline="true" :model="searchForm" class="search-form">
        <el-form-item label="参数类型">
          <el-select v-model="searchForm.codeType" placeholder="请选择类型" clearable>
            <el-option
              v-for="type in typeList"
              :key="type.codeType"
              :label="type.codeTypeName ? `${type.codeType} - ${type.codeTypeName}` : type.codeType"
              :value="type.codeType"
            ></el-option>
          </el-select>
        </el-form-item>
      ...
      <el-table :data="tableData" border style="width: 100%" v-loading="loading">
        ...
        <el-table-column label="状态" width="80">
          <template slot-scope="scope">
            <el-tag :type="(scope.row.isStop === 0 || scope.row.isStop === '0') ? 'success' : 'danger'">
              {{ (scope.row.isStop === 0 || scope.row.isStop === '0') ? '正常' : '停用' }}
            </el-tag>
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作" width="250">
          <template slot-scope="scope">
            ...
            <el-button 
              size="mini" 
              :type="scope.row.isStop === 0 || scope.row.isStop === '0' ? 'warning' : 'success'"
              :disabled="scope.row.codeType === 'SYSTEM_PARAM'"
              @click="handleToggleStatus(scope.row)"
            >
              {{ scope.row.isStop === 0 || scope.row.isStop === '0' ? '停用' : '启用' }}
            </el-button>
            <el-button 
              size="mini" 
              type="danger" 
      ...
      <el-form :model="form" :rules="rules" ref="form" label-width="120px">
        ...
        <el-form-item label="参数名称" prop="codeName" v-if="isEdit">
          <el-input 
            ...
            placeholder="请输入参数名称"
            :disabled="form.codeType === 'SYSTEM_PARAM'"
          ></el-input>
          <div v-if="form.codeType === 'SYSTEM_PARAM'" style="margin-top: 5px; color: #999; font-size: 12px;">
            系统参数不允许修改名称
          </div>
        </el-form-item>
      </el-form>
...
<script>
import { getCodeList, getCodeByType, getCodePage, getCodeByTypePage, saveCode, saveCodeBatch, updateCode, deleteCode, toggleCodeStatus } from '@/api/user'
  ...
  methods: {
    loadData() {
      ...
      const api = this.searchForm.codeType 
        ? getCodeByTypePage(this.searchForm.codeType, isStop, this.pagination.page, this.pagination.size)
        : getCodePage(isStop, this.pagination.page, this.pagination.size)
      api.then(response => {
      ...
      getCodeList().then(response => {
        if (response.code === 200) {
          ...
          ;(response.data || []).forEach(item => {
            if (item.codeType) {
              // 如果该类型还没有记录，或者当前记录的codeTypeName不为空，则更新
              if (!typeMap.has(item.codeType) || item.codeTypeName) {
                typeMap.set(item.codeType, {
          ...
          this.typeList = Array.from(typeMap.values())
        }
      })
    },
    handleSearch() {
    ...
    handleToggleStatus(row) {
      // 系统参数不允许停用/启用
      if (row.codeType === 'SYSTEM_PARAM') {
        this.$message.warning('系统参数不允许停用/启用')
        return
      }
      
      const action = row.isStop === 0 || row.isStop === '0' ? '停用' : '启用'
      this.$confirm(`确定要${action}该参数吗？`, '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        toggleCodeStatus(row.id).then(response => {
          if (response.code === 200) {
    ...
    handleDelete(row) {
      // 系统参数不允许删除
      if (row.codeType === 'SYSTEM_PARAM') {
        this.$message.warning('系统参数不允许删除')
        return
      }
      
      this.$confirm('确定要删除该参数吗？删除后无法恢复！', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
